...
function App() {
  ...
  const [balance,setBalance]= useState(0);//balance is initially zero
  useEffect(() => {
    async function init() {
...
//fetching account balance
  useEffect(()=>{
    ...
    async function getBalance(){
      if(account!== "Not connected"){
        const balanceWei = await web3.eth.getBalance(account);//await is used to interact with the blockchain
        const balanceEth = web3.utils.fromWei(balanceWei,"ether")
        setBalance(balanceEth);//balance = balanceEth
  ...
  return (
   ...
   <form className="label0" id="myForm">
      ...
      <p className="font" id ="man">For Manager</p>
      <Manager state={state} account={account}></Manager>
      {/* passing values i.e state and account  to the Manager component */}
      <p className="font" id ="inv">For Investors</p>
